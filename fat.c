...
uint8_t fsLoad(struct fsHandle *fsDesc, struct sDevice *device, uint8_t *memBuffer)
{
  ...
  if (*((uint16_t *)(fsDesc->buffer + 0x01FE)) != 0xAA55)
    return FS_DEVICE_ERROR;
  //Check sector size, fixed size of 2^SECTOR_SIZE allowed
  if (*((uint16_t *)(fsDesc->buffer + 0x0B)) != (1 << SECTOR_SIZE))
    return FS_DEVICE_ERROR;
  //Calculate sectors per cluster count
  tmp = *((uint8_t *)(fsDesc->buffer + 0x0D));
  fsDesc->clusterSize = 0;
  while (tmp >>= 1)
    fsDesc->clusterSize++;

  fsDesc->fatSector = *((uint16_t *)(fsDesc->buffer + 0x0E));
  fsDesc->dataSector = fsDesc->fatSector + *((uint8_t *)(fsDesc->buffer + 0x10)) * *((uint32_t *)(fsDesc->buffer + 0x24));
  fsDesc->rootCluster = *((uint32_t *)(fsDesc->buffer + 0x2C));
#ifdef FS_WRITE_ENABLED
  fsDesc->fatCount = *((uint8_t *)(fsDesc->buffer + 0x10));
  fsDesc->sectorsPerFAT = *((uint32_t *)(fsDesc->buffer + 0x24));
  fsDesc->clusterCount = ((*((uint32_t *)(fsDesc->buffer + 0x20)) - fsDesc->dataSector) >> fsDesc->clusterSize) + 2;
  fsDesc->infoSector = *((uint32_t *)(fsDesc->buffer + 0x30));
#ifdef DEBUG
  cout << "Sector number: " << *((uint32_t *)(fsDesc->buffer + 0x20)) << endl;
#endif
  if (readSector(fsDesc, fsDesc->infoSector))
    return FS_READ_ERROR;
  //Check info sector signatures (RRaA at 0x0000 and rrAa at 0x01E4)
  if ((*((uint32_t *)(fsDesc->buffer + 0x0000)) != 0x41615252) || (*((uint32_t *)(fsDesc->buffer + 0x01E4)) != 0x61417272))
    return FS_DEVICE_ERROR;
  fsDesc->lastAllocated = *((uint32_t *)(fsDesc->buffer + 0x1EC));
#ifdef DEBUG
  cout << "Free clusters: " << *((uint32_t *)(fsDesc->buffer + 0x1E8)) << endl;
#endif
#endif
  return FS_OK;
}
...
static uint8_t allocateCluster(struct fsHandle *fsDesc, uint32_t *cluster)
{
  ...
  for (; current != fsDesc->lastAllocated; current++)
  {
    ...
    if (*((uint32_t *)(fsDesc->buffer + offset)) == 0x00000000)
    {
      ...
      *((uint32_t *)(fsDesc->buffer + 0x1EC)) = current;
      fsDesc->lastAllocated = current;
      //Set free clusters count
      *((uint32_t *)(fsDesc->buffer + 0x1E8)) = *((uint32_t *)(fsDesc->buffer + 0x1E8)) - 1;
...
static uint8_t freeChain(struct fsHandle *fsDesc, uint32_t cluster)
{
  ...
  if (readSector(fsDesc, fsDesc->infoSector))
    return FS_READ_ERROR;
  //Set free clusters count
  *((uint32_t *)(fsDesc->buffer + 0x1E8)) = *((uint32_t *)(fsDesc->buffer + 0x1E8)) + freeCount;
  if (writeSector(fsDesc, fsDesc->infoSector))
    return FS_WRITE_ERROR;
  return FS_OK;
}
...
{
  uint16_t index;
  uint32_t tmp; //Stores first cluster of entry and entry description sector
  uint32_t parent;
  struct fsEntry item;
